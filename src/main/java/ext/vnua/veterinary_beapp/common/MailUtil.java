...
public class MailUtil {
    ...
    /**
     * Địa chỉ From mặc định. Nên khớp với tài khoản SMTP hoặc domain được phép gửi.
     * Cấu hình trong application.yml:
     *
     * app:
     *   mail:
     *     from: no-reply@thepro.com
     */
    @Value("${app.mail.from:no-reply@thepro.com}")
    private String defaultFrom;

    /**
     * Gửi email OTP HTML (UTF-8).
     *
     * @param email người nhận
     * @param otp   mã OTP 6 số
     * @throws EmailException nếu gửi thất bại (giữ nguyên root cause trong exception)
     */
    public void sendOtpEmail(String email, String otp) {
        try {
            MimeMessage mimeMessage = javaMailSender.createMimeMessage();
            // Dùng helper để set encoding thay vì tự set header
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, "UTF-8");
            helper.setTo(email);
            helper.setFrom(defaultFrom);
            helper.setSubject("The Pro - Mã xác thực tài khoản");
            helper.setText(buildOtpHtml(otp), true);

            javaMailSender.send(mimeMessage);
            log.info("OTP mail sent successfully. to={}", email);
        } catch (Exception e) {
            log.error("Send OTP mail failed. to={}, cause={}", email, e.getMessage(), e);
            throw new EmailException("Send OTP mail failed: " + e.getMessage() + e);
        }
    }
    ...
    private String buildOtpHtml(String otp) {
        // Lưu ý: vì dùng String#formatted, mọi dấu % trong HTML phải escape thành %%
        return """
